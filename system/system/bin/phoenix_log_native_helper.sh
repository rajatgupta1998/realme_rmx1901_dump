...
function phx_native_generate_keyfinfo()
{
    ...
    boot_stage=$(cat /proc/phoenix | grep "SET_BOOTSTAGE" | sed -n '$p' | cut -d "@" -f2)
...
{
    tmp_dir=$1
    cat ${MTK_LAST_KMSG_PROC} | grep "${KE_TAG}"
    if [ $? -eq 0 ]; then
        cat ${MTK_LAST_KMSG_PROC} > ${tmp_dir}/$(basename ${MTK_LAST_KMSG_PROC})
        cp -a ${MTK_PSTORE_DIR}/*  ${tmp_dir}/
    fi
}
...
function phx_log_native_helper_main()
{
    ...
    if [ "${phx_error}" == "ERROR_REBOOT_FROM_KE_SUCCESS" ]; then
